...
$.post(localhost + '/seller/pro/findOne', {
  ...
  produtsTypes.forEach(function (val, key) {
    ...
    $(".allInputs").append(
    '<div class="tjbianji">'+
        '<span class="tjspan">价格文案编辑</span>'+
        '<div class="input">'+
        '<input type="text" placeholder="例如特价优惠,二人团价等" value='+content.suffix+'>'+
        '</div>'+
        '</div>'+
        '<div class="timesInput">'+
        '<span class="tIspan">预上架的时间</span>'+
        '<div class="input">'+
        '<input type="date" name=""  value='+content.showDate.slice(0,10)+'>'+
        '<input type="number" placeholder="小时(24小时制)"min="0" step="1" value='+content.showDate.slice(10,13)+'> 时'+
        '<input type="number" placeholder="分钟" min="0" step="1" value='+content.showDate.slice(14,16)+'> 分'+
        '</div>'+
        '</div>'+
        '<div class="mInput">'+
        '<label for="checkBox">是否秒杀</label>'+
        '<input type="checkbox" id="checkBox" checked='+content.issecondkill +'>'+
        '</div>'+
        '<div class="allPdan">'+
        '<div class="pdan-start">'+
        '<span>秒杀开始时间</span>'+
        '<div class="input">'+
        '<input type="date" name=""  value='+content.secondKillStart.slice(0,10)+'>'+
        '<input type="number" placeholder="小时(24小时制)"min="0" step="1" value='+content.secondKillStart.slice(10,13)+'> 时'+
        '<input type="number" placeholder="分钟" min="0" step="1" value='+content.secondKillStart.slice(14,16)+'> 分'+
        '</div>'+
        '</div>'+
        '<div class="pdan-end">'+
        '<span>秒杀结束时间</span>'+
        '<div class="input">'+
        '<input type="date" name=""  value='+content.secondKillEnd.slice(0,10)+'>'+
        '<input type="number" placeholder="小时(24小时制)"min="0" step="1" value='+content.secondKillEnd.slice(10,13)+'> 时'+
        '<input type="number" placeholder="分钟" min="0" step="1" value='+content.secondKillEnd.slice(14,16)+'> 分'+
        '</div>'+
        '</div>'+
        '</div>')
  ...
  $('body').on('click', '.all-top .djia', function () {
    ...
    // 点击一键添加
    $('body').on('click', '.all-top .mjia', function () {
        $(this).find('input').css({
            'opacity': 1
        }).siblings('span').css({
            'opacity': 0
  ...
  $('body').on('blur', '.all-top .djia', function () {
    ...
    // 移出焦点进行赋值
    $('body').on('blur', '.all-top .mjia', function () {
        var val = $(this).find('input').val()
        var ind = $(this).parents('.all').index()
        $(this).find('input').css({
            'opacity': 0
        ...
        $('.all-con-right').eq(ind).find('.mjiage').val(val)
    })
