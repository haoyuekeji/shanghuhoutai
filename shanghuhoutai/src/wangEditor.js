...
var config = {
    ...
    // 默认菜单配置
    menus: ['head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo','familys'],
...
//             var configFamilys = editor.familys;
...
//                 }
//             */
//             configFamilys.forEach(function (v, k) {
...
Image.prototype = {
    ...
    _createInsertPanel: function _createInsertPanel() {
        ...
        var tabsConfig = [{
            title: '上传图片',
            tpl: '<div class="w-e-up-img-container">\n                    <div id="' + upTriggerId + '" class="w-e-up-btn">\n                        <i class="w-e-icon-upload2"></i>\n                    </div>\n                    <div style="display:none;">\n                        <input id="' + upFileId + '" type="file" multiple="multiple" accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp"  capture="camera"/>\n                    </div>\n                </div>',
            events: [{
                // 触发选择图片
...
function getChildrenJSON($elem) {
    ...
    $children.forEach(function (curElem) {
        ...
        // 文本节点
        if (nodeType === 3) {
            elemResult ={ type: 'text',text:curElem.textContent} ;
        }
        ...
        if (nodeType === 1) {
            elemResult = {};
            // tag
            elemResult.name = curElem.nodeName.toLowerCase();
          
            // attr
            elemResult.attrs={style:''}
            var attrData = ['padding:6px','box-sizing: border-box','max-width:100%','white-space: normal','word-wrap: break-word','word-break: break-all'];
            switch (curElem.nodeName.toLowerCase()) {
                case 'font':
                elemResult.name = 'span';
                    break;
                case 'u':
                elemResult.name = 'span';
                attrData.push('text-decoration:underline')
                    break;
                case "strike":
                elemResult.name = 'span';
                attrData.push('text-decoration:line-through')
                break;
                case "blockquote":
                attrData.push(' background: #f1f1f1;border-left: 8px solid #d0e5f2;padding: 5px 10px;margin: 10px 0;')
                break;
            ...
            for (var i = 0; i < attrListLength; i++) {
                ...
                var value 
                if(attr.name==='src'){
                    elemResult.attrs.src= attr.value
                }else{
                    value = attr.value.indexOf(':')=== -1 ? attr.name+':'+ attr.value : attr.value
                    attrData.push(
                        value
                    );
                }
            }
            elemResult.attrs.style=attrData.join(';')
            // children（递归）
        ...
        if (uploadImgServer && typeof uploadImgServer === 'string') {
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState === 4) {
                    ...
                    if (!hooks.customInsert && result.errno !== undefined) {
                        // hook - fail
                        if (hooks.fail && typeof hooks.fail === 'function') {
                            hooks.fail(xhr, editor, result);
                        }
                        ...
                        if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                            ...
                            data.split(',').forEach(function (link) {
