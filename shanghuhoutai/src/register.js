...
$('.zhanghao').eq(0).find('input').blur(function () {
  ...
  if (yonghu === '') {
    ...
    $.post(localhost + '/seller/findOne ', {sellerName: yonghu}, function (data) {
      if (data.message === '信息不存在') {
...
$('.mima').eq(1).find('input').blur(function () {
  ...
  if (mima1 !== mima2) {
    $('.mima').find('input').css({'color': 'red'})
    $('.hint').html('两次密码不一致！')
    mima2_ = false
  } else {
    $('.mima').find('input').css({'color': 'green'})
    $('.hint').html('')
    mima2_ = true
  }
})
...
$('.button').click(function () {
  ...
  if (zhanghao !== '' && mima1 !== '' && mima2 !== '' && APPID !== '' && authority !== '') {
    ...
    if (mima1_ === true && mima2_ === true && zhanghao_ === true && zhanghao1_ === true) {
      $.post(localhost + '/seller/reg', {
        sellerName: yonghu,
        sellerPass: mima1,
        sellerPhone: zhanghao,
        authority: authority,
        appId: APPID
      }, function (data) {
