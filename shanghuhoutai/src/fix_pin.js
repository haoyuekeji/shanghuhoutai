...
function img_number(arr) {
  if (token !== '8') {
    switch (arr) {
      case banner:
        maxFileCount1 = 5 - arr.length <= 0 ? 0 : 5 - arr.length
        if (maxFileCount1 === 0) {
          $('#loading').css({
            'display': 'none'
          })
        ...
        break
      case details_img:
        maxFileCount2 = 10 - arr.length <= 0 ? 0 : 5 - arr.length
        if (maxFileCount2 === 0) {
          $('#uploading').css({
            'display': 'none'
          })
...
// 获取商品数据并进行赋值
$.post(localhost + '/tuan/product/findone', {
  pid: pid,
  saleId: token,
  active: active,
  online_code: onlinecode
  ...
  var classify = content.types
  var produtsTypes = content.productsTypes
  var parameters = content.parameters
  var tuanNumbers = content.tuanNumbers
  var tuanTimes = content.tuanTimes
  banner = content.indexPic.split(',')
  details_img = content.detailPic.split(',')
  var color = []
  var json = {}
  var details_con = []
  img_number(banner)
  ...
  $('.kuanhao input').val(style)
  $('.classify input').val(classify)
  $('.canshu textarea').val(parameters)
  $('.pdan-num input').val(tuanNumbers)
  $('.pdan-time input').val(tuanTimes)
  if (content.startDate !== null) {
    var starttime = content.startDate
    $('.pdan-start').find('input').eq(0).val(starttime.split(' ')[0])
    $('.pdan-start').find('input').eq(1).val(starttime.split(' ')[1].split(':')[0])
    $('.pdan-start').find('input').eq(2).val(starttime.split(' ')[1].split(':')[1])
  }
  if (content.endDate !== null) {
    var endtime = content.endDate
    $('.pdan-end').find('input').eq(0).val(endtime.split(' ')[0])
    $('.pdan-end').find('input').eq(1).val(endtime.split(' ')[1].split(':')[0])
    $('.pdan-end').find('input').eq(2).val(endtime.split(' ')[1].split(':')[1])
  }
  ...
  color.forEach(function (val, key) {
    produtsTypes.forEach(function (val_, key_) {
      if (val === val_.color) {
        ...
        details_con[key_].newPrice = val_.newPrice
        details_con[key_].size = val_.size
        details_con[key_].oldPrice = val_.oldPrice
        details_con[key_].tuanPrice = val_.tuanPrice
      }
    })
  })
  color.forEach(function (val, key) {
    ...
    details_con.forEach(function (val_, key_) {
      if (val === val_.color) {
        $('.all').eq(key).find('.all-con-right ul').eq(0).append('<li>\n' +
          ...
          '                                </span></li>\n' +
          '                <li><input type="text" readonly class="clickdb jiage" value=' + details_con[key_].oldPrice + '></li>' +
          '<li><input type="text" readonly class="clickdb djiage" value=' + details_con[key_].newPrice + '></li>' +
          '<li><input type="text" readonly class="clickdb pjiage" value=' + details_con[key_].tuanPrice + '></li>' +
  ...
  $.post(localhost + '/tuan/types/list', {
    ...
    outline(res)
    if (res.data !== null) {
      var content = res.data.types.split(',')
          ...
          if ($('.pdan-start input').eq(1).val() - 0 >= 24 || $('.pdan-end input').eq(1).val() - 0 >= 24 || $('.pdan-start input').eq(2).val() - 0 >= 60 || $('.pdan-end input').eq(2).val() - 0 >= 60) {
            ...
            if (push_stu === 0) {
              ...
              $.post(localhost + '/tuan/product/save', {
                ...
                id: content.id,
                isTuan: content.isTuan,
                saleNum: content.saleNum,
