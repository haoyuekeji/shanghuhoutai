...
$.post(localhost + '/tuan/tusersale/index', {
  ...
  if (res.data !== null && res.data[0] !== null) {
    ...
    $('.loading-show-sure').click(function () {
      $.post(localhost + '/tuan/tusersale/update', {
        saleId: token,
        online_code: onlinecode,
        lunbo: banner_content.join(',')
      }, function (res) {
        if (res.message === '操作成功') {
          alert('更改成功！')
          $.post(localhost + '/tuan/tusersale/index', {
...
function addhtml_(i, content) {
  ...
  $('.moudle-body').append('<div class="moudle">' +
    ...
    '<div class="input">' +
    '<span>快递名称：</span>' +
    '<input type="text" calss="dname"  value=' + content[i].dname + '>' +
    '<span class="templatesId" style="display:none">' + content[i].id + '</span>' +
    '<ul></ul>' +
    '</div>' +
    '<div class="input">' +
    '<span>发货地址：</span>' +
    '<textarea name="" id="" cols="30" rows="2">' + content[i].sendAddress + '</textarea>' +
    '</div>' +
    '<div class="input">' +
    '<span>计价方式：</span>' +
    ...
    '<div class="freight">' +
    '<span>默认运费</span>' +
    '<input type="text" value=' + content[i].count + '>' +
    '<span class="jian">件</span>' +
    '<input type="text" value=' + content[i].moreCount + '>' +
    '<span>元，每增加</span>' +
    '<input type="text" value=' + content[i].price + '>' +
    '<span class="jian">件</span>' +
    '<span>，运费增加</span>' +
    '<input type="text" value=' + content[i].morePrice + '>' +
    '<span>元</span>' +
    '<span class="moudle-up" title="展开">' +
    ' <img src="images/arrow.png" alt="" width="100%">' +
    '</span>' +
    '<span class="post_dname" style="display:none">' + content[i].dname + '</span>' +
  ...
  for (var k = 0; k < content[i].templates.length; k++) {

    $('.moudle-con').append('<div class="moudle-con-body">' +
      '<div class="id">' + content[i].templates[k].id + '</div>' +
      '<div class="left">' +
      '<span class="address">' +
      content[i].templates[k].destinationAddress +
      '</span>' +
      '<span class="edit">编 辑</span>' +
      '</div>' +

      '<div class="con">' +
      '<input type="text" value=' + content[i].templates[k].count + '>' +
      '<input type="text" value=' + content[i].templates[k].price + '>' +
      '<input type="text" value=' + content[i].templates[k].moreCount + '>' +
      '<input type="text" value=' + content[i].templates[k].morePrice + '>' +
      ' </div>' +
...
$.post(localhost + '/tuan/tusersale/index', {
  ...
  if (res.data !== null && res.data[0] !== null) {
    ...
    var banner_pic = res.data[0].split(',')
...
// 获取邮箱信息
$.post(localhost + '/tuan/tusersale/findone', {
  saleId: token,
  online_code: onlinecode
}, function (data) {
  outline(data)
  if (data.data[0].email === null) {
    ...
    $('.email').find('input').val(data.data[0].email)
  }
  if (data.data[0].identification !== null) {
    var identification = data.data[0].identification.split('=')
    identification.forEach(function (val, key) {
      $('.authentication input').eq(key).val(val)
    })
  }
})
// 获取快递模板信息
$.post(localhost + '/tuan/tdelivertemplates/list', {
  ...
  for (var p = 0; p < content.length; p++) {
    $('.moudle-name').append('<span>' + content[p].dname + '</span>')
  }
...
$('.mima').find('input').eq(0).blur(function () {
  var mima = $(this).val()
  $.post(localhost + '/tuan/tusersale/checkpass', {
    saleId: token,
    pass: mima,
    online_code: onlinecode
  }, function (data) {
...
$('.yanzheng').click(function () {
  $.post(localhost + '/tuan/tusersale/phonecode',
    {
      saleId: token
    },
    function (res) {
...
//认证信息
$('.authentication-sure').click(function () {
  var len = $('.authentication input').length
  var authentication = []
  for (var i = 0; i < len; i++) {
    authentication.push($('.authentication input').eq(i).val())
  }
  $.post(localhost + '/tuan/tusersale/update', {
    saleId: token,
    identification: authentication.join('='),
    online_code: onlinecode
  }, function (res) {
    outline(res)
    if (res.message === "操作成功") {
...
$('#uploading').fileinput({
  language: 'zh', // 设置语言
  uploadUrl: localhost + '/tuan/tusersale/uploadfile', // 上传的地址
  ...
  if (confirm('您确定要删除本条详情吗？')) {
    if ($(this).parents('.moudle-con').siblings('.moudle-top').find('input').val() !== '' && id !== '') {
      $(this).parents('.moudle-con').find('.id').length > 1 ?
        $.post(localhost + '/tuan/tdelivertemplates/del_template', {
          id: id,
          saleId: token,
          online_code: onlinecode
        }, function (res) {
...
$('.moudle-body').on('click', '.moudle-del', function () {
  ...
  var templatesId = $(this).parents('.moudle').find('.templatesId').html()
  var that = $(this)
  if (confirm('您确定要删除模板吗？')) {
    if ($(this).parents('.freight').siblings('.moudle-top').find('input').val() !== '' && ind === 0) {
      $.post(localhost + '/tuan/tdelivertemplates/del_templates', {
        saleId: token,
        id: templatesId,
        online_code: onlinecode
      }, function (res) {
...
$('.moudle-body').on('click', '.edit', function () {
  ...
  for (var i = 0; i < tem.length; i++) {
    for (var j = 0; j < span_length; j++) {
      if (tem[i] === span_.eq(j).text()) {
        span_.eq(j).siblings('input').prop('checked', true)
      }
    }
  }
  for (var p = 0; p < $('.province-right').length; p++) {
    ...
    if (province_right_length === province_input_length) {
      $('.province-left').eq(p).find('input').prop('checked', true)
...
$('.cover-sure').click(function () {
  ...
  for (var i = 0; i < $('.province-right').find('input').length; i++) {
    if ($('.province-right').find('input').eq(i).prop('checked')) {
...
$('.moudle-body').on('click', '.gengxin', function () {
  ...
  $.post(localhost + '/tuan/tdelivertemplates/update_template', {
    saleId: token,
    id: id,
    destinationAddress: destination,
    count: account,
    price: price,
    moreCount: more_account,
...
$('.moudle-body').on('click', '.express-sure', function () {
  ...
  var templatesId = dom_.find('.templatesId').html()
  var freight_input = []
  var moudle_input = []
  express_all = []
  var dname = ''
  ...
  if (id_length === 0) {
    ...
    $.post(localhost + '/tuan/tdelivertemplates/save', {
      saleId: token,
      dname: dname,
      count: freight_input[0],
      price: freight_input[1],
      moreCount: freight_input[2],
      morePrice: freight_input[3],
      sendAddress: origin_address,
      priceType: price_type,
      lines: express_all.join('=='),
      online_code: onlinecode
    }, function (res) {
      outline(res)
      if (res.message === '操作成功') {
    ...
    for (var w = 0; w < dom_.find('.moudle-con-body').length; w++) {
      if (dom_.find('.moudle-con-body').eq(w).find('.id').text() === '') {
        ...
        $.post(localhost + '/tuan/tdelivertemplates/add_template', {
          saleId: token,
          destinationAddress: destination,
          count: account,
          price: price,
